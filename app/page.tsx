...
export default function QiXianPickleball() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (categoryList.length >= currentMax) {
      if (!window.confirm(`正取已滿，報名後將列為「備取第 ${categoryList.length - currentMax + 1} 位」，確定嗎？`)) return;
    }
      ...
      <div className="max-w-3xl mx-auto">
          ...
          <div className="flex justify-center gap-2 flex-wrap">
            {dayOptions.map(d => (
              <button 
                ...
                className={`px-4 py-2 rounded-xl font-bold transition-all ${selectedDay.key === d.key ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'bg-slate-800 text-slate-500 hover:text-slate-300'}`}
              >
                {d.label}
              </button>
            ))}
        ...
        <div className="flex gap-2 mb-6">
          {categories.map(cat => (
            <button 
              ...
              className={`flex-1 py-4 px-2 rounded-2xl font-black transition-all border-2 flex flex-col items-center justify-center ${activeTab === cat.label ? 'bg-slate-800 border-emerald-500 text-emerald-400 shadow-xl' : 'bg-slate-900 border-slate-800 text-slate-600'}`}
            >
              <span className="text-lg">{cat.label} ({cat.max}人)</span>
              <span className="text-[10px] opacity-70 uppercase tracking-widest mt-1">{cat.subLabel}</span>
            </button>
          ))}
        </div>
        ...
        <div className="grid md:grid-cols-5 gap-6">
          <form onSubmit={handleRegister} className="md:col-span-2 bg-slate-800 p-6 rounded-3xl h-fit space-y-4 border border-slate-700 shadow-2xl">
            ...
            <div>
              <label className="text-[10px] text-slate-500 ml-1">您的姓名 Name</label>
              <input type="text" required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder="請輸入姓名" className="w-full bg-slate-900 p-4 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 border border-slate-700 text-white" />
            </div>
            <div>
              <label className="text-[10px] text-slate-500 ml-1">4 碼密碼 Pwd (取消用)</label>
              <input type="password" maxLength={4} required value={formData.edit_code} onChange={e => setFormData({...formData, edit_code: e.target.value})} placeholder="設定密碼" className="w-full bg-slate-900 p-4 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 border border-slate-700 text-white" />
            </div>
            <button className="w-full bg-emerald-500 py-4 rounded-xl font-black text-xl hover:bg-emerald-400 transition-all shadow-lg active:scale-95 text-white">確認送出</button>
          </form>
          ...
          <div className="md:col-span-3">
            ...
            <div className="space-y-2">
              {currentList.map((p, i) => (
                <div key={p.id} className="bg-slate-800/40 p-4 rounded-2xl flex justify-between items-center border border-slate-800 hover:border-slate-700 transition-all">
